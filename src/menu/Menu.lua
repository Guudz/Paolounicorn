...
function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.Index = 1
    Menu.Sprite = { Dictionary = TextureDictionary or "char_unicorn", Texture = TextureName or "interaction_bgd", Color = { R = R, G = G, B = B, A = A } }
    Menu.Rectangle = nil
...
function RageUI.Menus:EditSpriteColor(R, G, B, A)
    if self.Sprite.Dictionary == "char_unicorn" then
...
function RageUI.Menus:SetSpriteBanner(TextureDictionary, Texture)
    self.Sprite = { Dictionary = TextureDictionary or "char_unicorn", Texture = Texture or "interaction_bgd" }
    self.Rectangle = nil
end
